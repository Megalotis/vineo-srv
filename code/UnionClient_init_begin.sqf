...
[] spawn {
	while {true} do {
		...
		if (isEngineOn _vehicleToFuel && ((driver _vehicleToFuel) == player) && !(isNull objectParent player)) then {
			_velocityOfVehicle = sqrt(((velocity _vehicleToFuel select 0)^2)+((velocity _vehicleToFuel select 1)^2))*3.6;
			_fuelConsumption = _velocityOfVehicle/110000 + 0.00005;
			if (_fuelConsumption > 0.001) then {
			...
			if (fuel _vehicleToFuel < 0.2 && fuel _vehicleToFuel > 0.18) then {
				hint "Ваш топливный бак пуст! Направляйтесь на ближайшую заправку";
			} else {
				if (fuel _vehicleToFuel < 0.03) then {
					hint "Надеюсь что вы еще можете передвигаться! Ваш бак совсем пуст..";
				};
			};	
		};
		sleep 2;
...
if (LIFE_SETTINGS(getNumber,"life_enableFatigue") isEqualTo 0) then {player enableFatigue true}; //false
