...
//запрещаем ограбление в людном месте
_nearUnits = (nearestObjects[player,["Man"],5]) arrayIntersect playableUnits;
if (count _nearUnits > 3) exitWith {hint "Здесь слишком много людей! Думаю стоит поехать на другую заправку."};
...
private _kassa = 10000 + round(random 100000);
