...
if(count _nearVehicles > 0) then {
	{
		...
		if(count _vehData  > 0 && (typeOf _x) in ["B_Truck_01_mover_F","C_Offroad_01_F","C_Van_01_transport_F","pop_volvo_camion_sin"]) then {
...
if (count (attachedObjects _vehicle) > 0) exitWith {hint "На вашей технике нет места для контейнера из-за модификации или другого контейнера..."};
