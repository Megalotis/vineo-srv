...
switch (playerSide) do {
	...
	case east: {
        ...
        //Drop fishing net
        life_actions = [player addAction[localize "STR_pAct_DropFishingNet",UnionClient_system_dropFishingNet,"",0,false,false,"",'
        (surfaceisWater (getPos vehicle player)) && (vehicle player isKindOf "Ship") && life_carryWeight < life_maxWeight && speed (vehicle player) < 2 && speed (vehicle player) > -1 && !life_net_dropped ']];
		
		//Drop Запуск добычи креветок и прочего говна
        life_actions = [player addAction[localize "STR_vInAct_WaterDeviceMine",UnionClient_system_catchFish,"",0,false,false,"",'
        (surfaceisWater (getPos vehicle player)) && (vehicle player isKindOf "Ship") && life_carryWeight < life_maxWeight && speed (vehicle player) < 2 && speed (vehicle player) > -1 && !life_net_dropped ']];
